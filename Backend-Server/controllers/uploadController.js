          ...
          if (translatedText) {
            fs.writeFile(filePath, translatedText, async (err) => {
              ...
              try {
                ...
                await replaceAudioInVideo(inputVideoPath, translatedAudioPath, translatedVideoPath);

                // Wait until the file is actually written before sending it
                fs.stat(translatedVideoPath, (err, stats) => {
                  if (err) {
                    console.error('Error locating translated video:', err);
                    return res.status(500).send('Error processing video.');
                  }
                  console.log('Replacement completed in Front-End successfully.');
                  res.setHeader('Content-Type', 'video/mp4');
                  res.sendFile(path.resolve(translatedVideoPath));
